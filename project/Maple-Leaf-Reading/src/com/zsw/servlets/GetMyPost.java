	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Post> posts=new ArrayList<>();
			try {
				...
				String sql = "select * from post where user_id='"+userId+"' order by post_id desc;";
				boolean b=false;
				...
				if(b) {
					...
					while(res.next()) {
						...
						//实例化post
						Post post=new Post(postId,content,photo,s,bookImg,bookName,bookAuthor,numberOFLikes,comments,Integer.valueOf(userId),userName);
						posts.add(post);		
					}
