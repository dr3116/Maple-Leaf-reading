...
@WebServlet("/PingLunListServlet")
public class PingLunListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       

    public PingLunListServlet() {
        super();
    }
...
//				System.out.println("   -   -   -   -   -   -   -   -   -   -   -   ");
				...
				studentsNumber=studentService.getPingLunsNumber();//-----		
				...
				if(order.equals("delete")) {//删除图书，	参数  deleteBookName
					try {
						...
						studentService.deletePingLunById(userID);
						studentsNumber--;
				...
				//多表查询
				String sql="select * from book_review,book,user where book_review.user_id=user.user_id and book_review.book_name=book.book_name limit "+num1+","+num2;	
				//判断是不是精确查询
				if(searchName!="" && !searchName.equals("")) {
					sql="select * from book_review,book,user where book_review.user_id=user.user_id and book_review.book_name=book.book_name like'%"+searchName+"%'";
					System.out.println("search SQL="+sql);
				}
				List<PingLun> students = studentService.getPingLuns(sql);	//-------
