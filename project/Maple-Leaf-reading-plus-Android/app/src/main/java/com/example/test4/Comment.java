...
//                Log.e("设置listView","成功");
    ...
    private void  setView(){
        ...
        Glide.with(this)
                .load(ConfigUtil.SERVER_ADDR+"images/post/"+post.getPhoto())
                .apply(requestOptions)
                .into(postImg);
        Glide.with(this)
                .load(ConfigUtil.SERVER_ADDR+"images/student/"+post.getUserId()+".jpg")
                .apply(requestOptions)
                .into(posterPhoto);
        Glide.with(this)
                .load(ConfigUtil.SERVER_ADDR+"images/book/"+post.getBookImg())
                .apply(requestOptions)
                .into(bookImg);
        ...
        public void run() {
            try {
                ...
                Request request = new Request.Builder()
                        // 指定访问的服务器地址
                        .post(body)
                        .url(ConfigUtil.SERVER_ADDR+"APP/GetPostComment")
                        .build();
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    FormBody body = builder.build();
                    Log.e("Comment.java: ",userId+"-"+post.getPostId()+"-"+inputComment.getText().toString());

                    Request request = new Request.Builder()
                            // 指定访问的服务器地址
                            .post(body)
                            .url(ConfigUtil.SERVER_ADDR+"APP/UpLoadPostComment")
                            .build();
                    Response response = client.newCall(request).execute();
                    Log.e("重新获取评论","开始");
                    getpostComment();
